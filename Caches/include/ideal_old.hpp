...
template <typename PageT, typename KeyT=int> struct ideal_old
{
  size_t sz_;
  ...
  size_t find_first_occurence(KeyT key, const std::vector<KeyT>& future, size_t now) const
  {
  ...
  CacheFarthest find_farthest_in_cache(const std::vector<KeyT>& future, size_t now) const
  {
  ...
  template<typename F>
  void full_cache_update(const std::vector<KeyT>& future, size_t now, F slow_get_page)
  {
    assert(now < future.size());
  ...
  template<typename F> bool lookup_update(KeyT key, F slow_get_page, const std::vector<PageT>& future, size_t now)
  {
    assert(now < future.size());
